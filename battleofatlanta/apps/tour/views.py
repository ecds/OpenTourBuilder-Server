...
def tour_stop_detail(request, slug, page):
    ...
    return render_to_response("tour/tour_stop-detail.html", {
            'tour_stop': page[0],
            'images': page[0].tourstopmedia_set.all().filter(media_type_id='2'),
            'audio': page[0].tourstopmedia_set.all().filter(media_type_id='3'),
            'videos': page[0].tourstopmedia_set.all().filter(media_type_id='4'),
            'page': page,
	    'tour_version': tour_version,
            'switch': switch,
        }, context_instance=RequestContext(request))
